...
namespace MTurk.Algo
{
    public class SubHistory
    {
        ...
        private static int TurksLastConcession(int i, float[] moves)
        {
            i--;
            int res = -1;
            for (; i - 2 >= 0; i -= 2)
        ...
        private static int MachinesLastConcession(int i, float[] moves)
        {
            int res = -1;
            for (; i - 2 >= 0; i -= 2)
            {
        ...
        private static float TurksFirst(float[] moves, bool machineStarts)
        {
            if (moves.Length == 1)
                return -1;
        ...
        private static float MachinesFirst(float[] moves, bool machineStarts)
        {
            Debug.Assert(moves.Length > (machineStarts ? 0 : 1));
            return moves[machineStarts ? 0 : 1];
        }
        private static float MachinesLast(int i, float[] moves, bool machineStarts)
        {
            Debug.Assert(machineStarts ? i % 2 == 0 : i % 2 == 1);
            return moves[i];
        }

        private static float TurksLast(int i, float[] moves, bool machineStarts)
        {
        ...
        private static float MachinesLast1(int i, float[] moves)
        {
            if (i < 2)
                return -1;
            else
        ...
        private static float TurksLast1(int i, float[] moves)
        {
            if (i < 3)
                return -1;
            else
